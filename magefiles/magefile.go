...
func buildTags() string {
	...
	var tags []string
	if mode == "cgo" {
		tags = append(tags, "aho_corasick_cgo")
	}

	return strings.Join(tags, ",")
}

// Test runs unit tests - by default, it uses wazero; set WASI_TEST_MODE=cgo or WASI_TEST_MODE=tinygo to use either, or
...
func Format() error {
	...
	if err := sh.RunV("go", "run", fmt.Sprintf("github.com/rinchsan/gosimports/cmd/gosimports@%s", verGosImports), "-w",
		"-local", "github.com/wasilibs/go-aho-corasick",
		"."); err != nil {
		return nil
	}
	return nil
...
func UpdateLibs() error {
	if err := sh.RunV("docker", "build", "-t", "ghcr.io/wasilibs/go-aho-corasick/buildtools-aho-corasick", "-f", filepath.Join("buildtools", "aho-corasick", "Dockerfile"), "."); err != nil {
		return err
	}
	wd, err := os.Getwd()
	if err != nil {
	...
	return sh.RunV("docker", "run", "-it", "--rm", "-v", fmt.Sprintf("%s:/out", filepath.Join(wd, "wasm")), "ghcr.io/wasilibs/go-aho-corasick/buildtools-aho-corasick")
}
...
// BenchCGO runs benchmarks with aho-corasick accessed using cgo. A C++ toolchain and libaho-corasick must be installed to run.
func BenchCGO() error {
...
// BenchAll runs all benchmark types and outputs with benchstat. A C++ toolchain and libaho-corasick must be installed to run.
func BenchAll() error {
...
// WAFBenchCGO runs benchmarks with aho-corasick accessed using cgo. A C++ toolchain and libaho-corasick must be installed to run.
...
// WAFBenchAll runs all benchmark types and outputs with benchstat. A C++ toolchain and libaho-corasick must be installed to run.
...
func benchArgs(pkg string, count int, mode benchMode) []string {
	...
	switch mode {
	case benchModeCGO:
		args = append(args, "-tags=aho_corasick_cgo")
	case benchModeSTDLib:
		args = append(args, "-tags=aho_corasick_bench_stdlib")
	}
